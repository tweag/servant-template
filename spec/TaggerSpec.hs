...
registerUser env login' = runClientM (apiClient.public.authentication.register login') env

loginUser :: ClientEnv -> Credentials -> IO (Either ClientError (Id User, Token))
loginUser env login' = do
  userId <- registerUser env login'
  token <- runClientM (apiClient.public.authentication.login login') env
  pure $ (,) <$> userId <*> token
...
addUserContent env token' content = do
  let authClient = apiClient.secure (toServantToken token')
  runClientM (authClient.content.addContent content) env

getUserContents :: ClientEnv -> Token -> [Tag] -> IO (Either ClientError [Owned (Content Tag)])
getUserContents env token tags = do
  let authClient = apiClient.secure (toServantToken token)
  runClientM (authClient.content.getContents tags) env
  ...
  describe "Tagger" $ do
    ...
    describe "login" $ do
      ...
      it "does not generate a token for a non registered user" $ \port -> do
        let loginOperation = apiClient.public.authentication.login
