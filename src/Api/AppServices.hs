...
connectedContentRepository logHandle = hoistContentRepository (eitherTToHandler $ (>> throwError err500) . logError logHandle . show)
...
connectedUserRepository logHandle = hoistUserRepository $ eitherTToHandler handleUserRepositoryError
  where
...
encryptedPasswordManager logHandle = hoistPasswordManager (eitherTToHandler handlePasswordManagerError) . bcryptPasswordManager
  where
